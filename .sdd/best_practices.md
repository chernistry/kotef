...
- **Cloud/Vendor Updates**: OpenAI, Anthropic, etc., continue to update their models (GPT-4 turbo, Claude-v2, etc.) and pricing. By late 2025, using smaller fine-tuned models for coding tasks is becoming viable (e.g. Code Llama 2 or OpenAI function-calling models). Our design should allow swapping out the LLM with minimal changes. Also, cloud providers (AWS, GCP) have introduced their agent services (Azure OpenAI has orchestration, Google’s Gemini API supports tool use[\[23\]](https://oyelabs.com/langgraph-vs-crewai-vs-openai-swarm-ai-agent-framework/#:~:text=LangGraph%20vs%20CrewAI%20vs%20OpenAI,to%20cater%20to%20specific)). We remain cloud-agnostic but might integrate with these if self-hosting becomes burdensome.
