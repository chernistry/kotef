...
export function buildKotefGraph(cfg, deps = {}) {
    ...
    graph.addNode("planner", plannerNode(cfg, chatFn));
    graph.addNode("researcher", researcherNode(cfg));
    graph.addNode("coder", coderNode(cfg, chatFn));
    graph.addNode("verifier", verifierNode(cfg));
    // Add edges
    graph.addEdge(START, "planner");
    // Planner decides where to go
    graph.addConditionalEdges("planner", (state) => {
        const next = state.plan?.next;
        if (next === 'researcher')
            return 'researcher';
        if (next === 'coder')
            return 'coder';
    ...
    graph.addEdge("coder", "verifier");
    // Verifier goes to Planner (if failed) or End (if passed)
    graph.addConditionalEdges("verifier", (state) => state.done ? "end" : "planner", {
        end: END,
        planner: "planner"
    });
